#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2020/3/12 15:29
# @Author  : Zhangyp
# @File    : RegisterScenariosScript.py
# @Software: PyCharm
...
class RegisterTaskSet(TaskSet):
	# def on_start(self):
	# 	"""执行任务前执行，每个locust执行一次"""
	# 	pass
	#
	# # print('任务开始')
	#
	# def on_stop(self):
	# 	"""执行任务后执行，每个locust执行一次"""
	# 	pass
	...
	# @seq_task(2)
	@task(1)
	class RegCanOrder(TaskSequence):
		"""改变检查类型"""
	...
	# @seq_task(2)
	@task(1)
	class OrderSearch(TaskSequence):
		"""今日患者查询"""
...
def start():
	...
	cli = "locust -f RegisterScenariosScript.py -H %s --web-host %s -P %s" % (HOST, WEB, 8188)
	try:
